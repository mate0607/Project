                ...
                @forelse($appointments as $appointment)
                    <tr>
                        ...
                        <td>
                            ...
                            @else
                                <span class="app-status" style="background: rgba(148,163,184,0.12); color: #94a3b8; border-color: rgba(148,163,184,0.3);">
                                    Előjegyezve
                                </span>
                            @endif
                        </td>
                        <td>
                            ...
                            <form method="POST" action="{{ route('admin.appointments.destroy', $appointment) }}" style="margin:0;">
                                @csrf
                                @method('DELETE')
                                <button type="button" title="Törlés"
                                    onclick="if(confirm('Biztosan törölni szeretnéd?\n\nFelhasználó: {{ addslashes($appointment->user?->name ?? '—') }}\nAutó: {{ addslashes($appointment->car?->make_model ?? '—') }}\nDátum: {{ $appointment->date instanceof \Carbon\Carbon ? $appointment->date->format('Y-m-d') : $appointment->date }}\nIdő: {{ $appointment->time }}')) this.closest('form').submit();"
                                    style="display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;border-radius:8px;background:rgba(239,68,68,0.18);border:1px solid rgba(239,68,68,0.35);cursor:pointer;transition:background 0.2s;">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#f87171" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
                                </button>
                            </form>
                            </div>
                        </td>
