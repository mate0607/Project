...
class DashboardController extends Controller
{
    public function admin()
    {
        ...
        $upcomingAppointments = Appointment::with(['car', 'user'])
            ->whereRaw("TIMESTAMP(`date`, `time`) >= ?", [now()->format('Y-m-d H:i:s')])
            ->orderBy('date')
            ->orderBy('time')
            ->limit(8)
            ->get();
    ...
    public function user()
    {
        ...
        $issuesCount = Issue::whereHas('car', function ($query) use ($userId) {
